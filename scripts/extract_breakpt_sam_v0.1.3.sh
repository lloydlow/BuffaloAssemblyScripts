...
if [[ $# -lt 3 ]] 
then
        echo "USAGE: extract_breakpt_sam breakpt in.Chic.bam in.PacBio.bam"
        exit 1
fi
...
do
	...
	#echo "$COUNTER"
	#extract coordinates based on breakpt
	...
	#mv $prefixName".png" $PATH2BUFFASSEMBLYDATA"/"$prefixName"$COUNTER"".png"
	#mv $prefixName"_break.tsv" $prefixName"$COUNTER""_break.tsv"
done < "$1"

#cat *tsv > tmp; rm *tsv; mv tmp all_breakpt.tsv
#mv all_breakpt.tsv $PATH2BUFFASSEMBLYDATA"/"
