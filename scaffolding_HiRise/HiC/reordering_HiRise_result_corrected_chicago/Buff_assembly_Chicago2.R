...
#The above is note on the origin of the script but it is now adapted for checking
#chicago corrected HiRise result

#read in *input_breaks.txt
inputBreaks <- read.table("data/water_buffalo_20Sep2017_imKQY.input_breaks.txt",
...
#read in *table.txt
table <- read.table("data/water_buffalo_20Sep2017_imKQY.table.txt",stringsAsFactors = FALSE)
...
#Output the 743 fragments that have been labeled on their break and join status
#however, whether the break is to be trusted is not decided.
write.csv(table.new,"hiC_743_fragments.csv")
write.table(table.new,file="hiC_743_fragments.tsv",sep="\t",row.names = FALSE,col.names = FALSE)

# output joined_both and single_broken